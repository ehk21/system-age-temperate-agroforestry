...
# save model results
################################################################################
# write.csv(variables, "results/16_sensitivity-bee-alpha-no-weightings-vars.csv", row.names=F)
# write.csv(models_df, "results/16_sensitivity-bee-alpha-no-weightings-models.csv", row.names=F)
...
alpha_plot <-
  ...
  ggtitle("Sensitivity test A: solitary bee alpha diversity")


ggsave(plot=alpha_plot, filename="figs/16_sensitivity-solitaries-alpha-no-weightings.pdf", width=22, height=14, units="cm", dpi=300)
