...
#############
# solitary bee alpha diversity
#############

# load data
sol.alpha <- read.csv("data/cleaned/06_solitary-alpha-div.csv")
pred <- read.csv("results/08_bee-alpha-predictions.csv")
...
sol.alpha$Site <- factor(sol.alpha$Site)
...
sol.alpha.plot <-
  ggplot(sol.alpha, aes(x = Age, y = qD)) +
  geom_point(
  ...
  facet_grid2(factor(Order.q,
    ...
    facetted_pos_scales(
      y = list(
        scale_y_continuous(limits = c(0, 32), breaks = seq(0, 35, by = 5)),  # Row 1 - tall
        scale_y_continuous(limits = c(0, 22), breaks = seq(0, 25, by = 5)),
        scale_y_continuous(limits = c(0, 22), breaks = seq(0, 25, by = 5))  # Row 2 - medium
      )
    ) +
  ...
  fill = guide_legend(
    ...
    scale_x_continuous(breaks=seq(min(sol.alpha$Age), max(sol.alpha$Age), by=2))
  
# ggsave(plot=sol.alpha.plot, filename="figs/XX_solitaries-alpha-div.pdf", width=20, height=20, units="cm", dpi=300)
...
# load data
bee_alpha <- read.csv("results/08_bee-alpha-vars.csv")
...
# reorder and rename parameter levels
...
# ggsave(plot=alpha_plot, filename="figs/XX_solitaries-alpha-beta-estimates.pdf", width=22, height=14, units="cm", dpi=300)
...
ggsave(plot=combined.plot, filename="figs/12_solitaries-alpha-combined.pdf", width=24, height=30, units="cm", dpi=300)
