...
model_avg_q0 <- model.avg(model_subset_q0)
model_avg_fixed_q0 <- model.avg(model_subset_fixed_q0)
...
# Average top models with ΔAICc < 4
model_avg_q1 <- model.avg(model_subset_q1)
model_avg_fixed_q1 <- model.avg(model_subset_fixed_q1)
...
# Average top models with ΔAICc < 4
model_avg_q2 <- model.avg(model_subset_q2)
model_avg_fixed_q2 <- model.avg(model_subset_fixed_q2)
...
# save model results
################################################################################
# write.csv(variables, "results/17_sensitivity-bee-func-no-weightings-vars.csv", row.names=F)
# write.csv(models_df, "results/17_sensitivity-bee-func-no-weightings-models.csv", row.names=F)
...
func_plot <-
  ...
  ggtitle("Sensitivity test A: solitary bee functional diversity")


ggsave(plot=func_plot, filename="figs/17_sensitivity-solitaries-func-no-weightings.pdf", width=22, height=14, units="cm", dpi=300)
